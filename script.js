...
$(document).ready(() => {
  ...
  let show_text = () => {
    ...
    let css_change = (elem, win, invert = false) => {
      ...
      elem.css(
        "color",
        "color-mix(in srgb, var(--final-color) " +
          (100 * Math.pow(2, curr_pos * sp)) / Math.pow(2, sp) +
          "%, var(--initial-color))"
      );
    };
  ...
  setTimeout(() => {
    $(".img").addClass("imganim");
  }, delayFn(word_list.length / 2));

  setTimeout(() => {
    $(".cont").css("opacity", "1");
  }, delayFn(word_list.length / 2) + 2000);
